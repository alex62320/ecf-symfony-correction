...
class LoginFormAuthenticator extends AbstractLoginFormAuthenticator
{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, string $firewallName): ?Response
    {
        ...
        elseif(in_array('ROLE_EMPRUNTEUR', $roles)){
            ...
            $emprunteur = $this->emprunteurRepository->findByUser($user);
            return new RedirectResponse($this->urlGenerator->generate('app_emprunteur_show', ['id' => $emprunteur->getId(),]));
        }

        // en dernier lieu, l'utilisateur est renvoyer vers la page d'acceuil
